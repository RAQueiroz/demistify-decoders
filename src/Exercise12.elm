...
import Json.Decode exposing (Decoder, andThen, fail, field, int, lazy, list, map, map2, oneOf, string, succeed)
...
decoder =
    oneOf
        [ field "value" int
            |> map2 Leaf (field "name" string)
        , field "children" (list (lazy (\_ -> decoder)))
            |> map2 Branch (field "name" string)
        ]
